...
{
    public class GLeagueMapper
    {
        public async Task Map(string scrapedPlayersPath, string mongoPlayersPath, string boxScoresPath, string fileName)
        {
            var scrapedPlayers = new DataImporter<BBRefPlayer, BBRefPlayerMap>().LoadData(scrapedPlayersPath, ";")
                .ToList();
            var mongoPlayers = new DataImporter<MongoPlayerDto, MongoPlayerDtoMap>().LoadData(mongoPlayersPath, ";")
                .ToList();
            ...
            foreach (var group in scrapedPlayers.GroupBy(x => x.Url))
            {
                foreach (var bbRefPlayer in group)
                {
                    ...
                    {
                        new GLeaguePlayerMapper(pc, possiblePlayers, possibleBoxScores).ApplyMap();
